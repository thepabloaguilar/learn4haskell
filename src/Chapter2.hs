...
subList start end l
  | start < 0 || end < 0 = []
  | end < start = []
  | otherwise = take (end - start + 1) (drop start l)
...
rotate n _ | n < 0 = []
rotate _ [] = []
rotate n l = drop numbersToTake l ++ take numbersToTake (cycle l)
  where
    {- |
      If rotation is greater than the length of the list we need to cut it!
      To do this we can simply use the rest of the division, for example, a list
      that has 5 elements and the user call `rotate` passing a rotation of 16 we do
      `16 / 5` and get the rest of the division which is `1`.
      Another way of thinking about this is, "how many complete rotations we have?",
      where a complete rotation is when the rotation number is divisible by the list lenght
      which means the rest of the division is the "pending" rotation to do! Example:
      * (one complete rotation, rest is 0) rotate 4 [1..4] -> [1,2,3,4]
      * (three complete rotation, rest is 1) rotate 16 [1..5] -> [1,2,3,4]
    -}
    numbersToTake = rem n (length l)
